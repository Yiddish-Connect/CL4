...
class _Step1 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<PreferenceProvider>(
      builder: (BuildContext context, PreferenceProvider preferenceProvider,
          Widget? child) {
        ...
        nameController.text =
            Provider.of<PreferenceProvider>(context, listen: false).name;
            ...
            children: List.generate(_interests.length, (index) {
              return Consumer<PreferenceProvider>(
                builder: (BuildContext context,
                    PreferenceProvider preferenceProvider, Widget? child) {
                  ...
                  return ChoiceChip(
                    ...
                    onSelected: (selected) {
                      if (selected &&
                          preferenceProvider.selectedInterests.length < 5) {
...
class _Step4State extends State<_Step4> {
  ...
  List<Uint8List?> _webImages =
      List<Uint8List?>.filled(6, null); // List for web images
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ConstrainedBox(
              constraints:
                  BoxConstraints(maxWidth: 400), // Set max width for the grid
              child: Padding(
                padding: EdgeInsets.all(10.0),
                child: LayoutBuilder(
                  builder: (context, constraints) {
                    final itemSize = (constraints.maxWidth - 3 * 3) /
                        3; // Calculate item size
  ...
  Future<void> uploadImage(dynamic file, String userId) async {
    try {
      ...
      Reference storageRef =
          FirebaseStorage.instance.ref().child('user_$userId/$fileName');
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Stack(
        children: [
          if (kIsWeb &&
              webImage != null) // Check if it's web and web image is available
            ClipRRect(
              borderRadius: BorderRadius.circular(20),
              child: Image.memory(
                webImage!,
